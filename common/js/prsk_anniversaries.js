...
function displayAnniversaryYearCount(){
 ...
 for(s=1;s<=1231;s++){
  if(document.getElementById("d_"+s) != null){
   ...
   var c = $(document.getElementById("d_"+s)).find("[class='chara_vs']");
   ...
   for(h=0;h<c.length;h++){
    if(typeof c[h].dataset["birthyear"] == "undefined"){continue;}
    var age_c = y-(new Number(c[h].dataset["birthyear"]));
    var checksum_c = (((new Date()).getMonth()+1) * 100)+((new Date()).getDate());
    if(checksum_c < s){age_c--;}
    c[h].getElementsByClassName("age_now")[0].innerHTML = `, ${c[h].dataset["birthyear"]}년 출시/${age_c}주년 경과`;
   }
...
var cMessage = "세카이에 오신 것을 환영합니다.";
console.log("%c "+cMessage,"color:#33CCBA;font-weight:bold;");